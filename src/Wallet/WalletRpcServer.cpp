...
namespace Tools {
  ...
  { "rpc-bind-ssl-port", "Starts wallet as RPC server for wallet operations, sets bind SSL port for server.", DEFAULT_RPC_SSL_PORT };
  ...
  bool wallet_rpc_server::init(const boost::program_options::variables_map& vm)
  {
    ...
    else
    {
      ...
      if (m_enable_ssl) {
        ...
        {
          logger((Logging::Level) ERROR, BRIGHT_RED) << "Starting RPC SSL server was canceled because certificate file(s) could not be found";
        }
      }
      return true;
    }
