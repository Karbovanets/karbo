...
namespace
{
    ...
    const std::string TESTNET_DB_NAME = "testnet_LevelDB";
}

LevelDBWrapper::LevelDBWrapper(Logging::ILogger& logger, const DataBaseConfig &config):
...
void LevelDBWrapper::init()
{
    ...
    dbOptions.compression = m_config.getCompressionEnabled()
    ...
    dbOptions.max_file_size = m_config.getMaxFileSize();
    ...
    dbOptions.write_buffer_size =  static_cast<size_t>(m_config.getWriteBufferSize());

    // Number of open files that can be used by the DB.  You may need to
    // increase this if your database has a large working set (budget
    // one open file per 2MB of working set).
    dbOptions.max_open_files = m_config.getMaxOpenFiles();

    dbOptions.block_cache = leveldb::NewLRUCache(m_config.getReadCacheSize());
...
std::error_code LevelDBWrapper::writeSync(IWriteBatch& batch)
{
    if (state.load() != INITIALIZED)
    {
        throw std::system_error(make_error_code(CryptoNote::error::DataBaseErrorCodes::NOT_INITIALIZED));
